...
public final class SDTParser implements Parser {
	...
	public Transform parse(Reader input) throws IOException, ParseException, java.text.ParseException {
		...
		dNode sdt = SDA.parser().parse(input);
		return parse(sdt);
	}
	 ...
	 */
	public static Transform parse(dNode sdt) throws ParseException {
		/*
		...
		for (Node node : sdt.find(n -> ! n.isLeaf()))
			transform.add(parseStatement((dNode) node)); // parse and add child statements
	 ...
	 */
	private static Statement parseStatement(dNode sdt) throws ParseException {
		/*
	 ...
	 */
	private static PrintStatement parsePrintOrPrintLn(dNode sdt, boolean addEOL) throws ParseException {
		/*
		...
		dNode value = getAttribute(sdt, Attribute.VALUE, true);
	 ...
	 */
	private static VariableStatement parseVariableOrParam(dNode sdt) throws ParseException {
		/*
		...
		if ( isParam ) {
			...
			if (parent.find(n -> 
				n.getName().equals(Statements.PARAM.tag) 
					&& ((dNode) n).getValue().equals(varname)).size() > 1)
		...
		dNode select = getAttribute(sdt, Attribute.SELECT, true);
	 ...
	 */
	private static ForEachStatement parseForEach(dNode sdt) throws ParseException {
		/*
		...
		for (Node node : sdt.nodes()) // parse and add child statements
			statement.add(parseStatement((dNode) node));
	 ...
	 * a IfStatement.
	 */
	private static IfStatement parseIf(dNode sdt) throws ParseException {
		/*
	 ...
	 */
	private static ChooseStatement parseChoose(dNode sdt) throws ParseException {
		/*
		...
		for (Node node : sdt.nodes()) {
			...
			++i;
			Statement substat = parseStatement((dNode) node);
	 ...
	 */
	private static WhenStatement parseWhen(dNode sdt) throws ParseException {
		/*
	 ...
	 */
	private static OtherwiseStatement parseOtherwise(dNode sdt) throws ParseException {
		/*
	 ...
	 */
	private static Statement parseNode(dNode sdt) throws ParseException {
		/*
		...
		checkAttributes(sdt, Arrays.asList(Attribute.VALUE));
		final dNode value = getAttribute(sdt, Attribute.VALUE, false);
		...
		for (Node node : sdt.find(n -> ! n.isLeaf()))
			stat.add(parseStatement((dNode) node)); // parse and add child statements
	 ...
	 */
	private static CopyStatement parseCopy(dNode sdt) throws ParseException {
		/*
	 ...
	 */
	private static SDAXPath xpathFromNode(dNode node) throws ParseException {
	 ...
	 */
	private static void checkStatements(dNode sdt, List<Statements> allowed) throws ParseException {
	 ...
	 */
	private static void checkAttributes(dNode sdt, List<Attribute> allowed) throws ParseException {
	 ...
	 */
	private static dNode getAttribute(dNode sdt, Attribute attribute, Boolean required) throws ParseException {

		List<dNode> alist = sdt.find(n -> n.isLeaf() && n.getName().equals(attribute.tag));
		int size = alist.size();
